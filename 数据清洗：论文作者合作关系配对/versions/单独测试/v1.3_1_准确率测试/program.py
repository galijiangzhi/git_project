...
def str_process(data):#将表格中的作者信息拆分
    #print(data)
    data_pro = data.replace('，',';').replace(',',';')#逗号变成分号
    data_part = data_pro.split(';')
    #print(data_pro)
    #将单一作者加入字典，如果重名则不添加
...
def create_dict(data):
    for i in range(len(data)):
        data_part = data[i].replace('，',';').replace(' ','').replace(',',';').split(';')
        #print(f"data_part 的数值是：{data_part}")
        while '' in data_part:
            data_part.remove('')
        for j in range(len(data_part)):#j是作者在data文件中的名字
            for k in range(len(author_list)):#k是作者在列表中的名字
                if(string_similar(str_change(data_part[j]).replace(' ',''),str_change(author_list[k])))> 0.89: #超找到之后使用k代替j
...
#print(data)
#print(author_list)
copy_list()#向excal里写入数据
wb.save("task_data.xlsx")#保存工作簿
'''create_dict(data)#根据列表创建出字典
print("字典创建完毕")
type_in_dict()#将data中的数据整理到字典
#print(author_dict)
print("#################################")
print(author_dict)
print("正在向excal写入数据")
