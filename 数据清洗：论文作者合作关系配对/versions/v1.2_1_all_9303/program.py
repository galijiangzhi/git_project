...
def type_in_dict(data):
    ...
    for i in range(len(data)):
        data_part = data[i].replace('，',';').split(';')
            ...
            for k in range(len(author_list)):#k是作者在列表中的名字
                if(string_similar(str_change(data_part[j]).replace(' ',''),str_change(author_list[k])))> 0.89: #超找到之后使用k代替j
                    ...
                    if author_list[k] not in author_dict:
                        ...
                        l=len(data_part)-1
                        while(l>j):    
                            if (l>j and (str_find(data_part[l]) not in author_dict[author_list[k]])and(str_find(data_part[l]) != author_list[k])):
    ...
    for i in range(len(author_list)):
        ...
        if len(author_dict[a]) >0:
            for x in range(len(author_dict[a])):
                ...
                d+=1
#将data.xlsx->sheet1->(b2:b9193)中的人物数据（排序后排除空项，b1,b8933)
...
print("正在提取数据")
data = sht.range("b1:b8933").value#提取数据
